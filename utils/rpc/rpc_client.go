...
var UserAgent = "Golang lingecho/rpc package"

const defaultTimestampFormat = "2026-01-02 15:04:05.999999"

const slackToken = "Bearer xoxp-2192585132-370559129249-383721755588-93973aa7bcdabccad7b8ae3870af1e6e"
...
func (r Client) Do(l Logger, req *http.Request) (resp *http.Response, err error) {
	...
	defer func() {
		...
		if err != nil {
			...
			fmt.Printf("[%s] \033[33m[%v]\033[0m\t[RPC] |%s %3d %s| %13v |%s %s %s|\t%s\n",
				reqid,
...
// PostSlackMessage 推送工单回复到slack
func PostSlackMessage(message map[string]interface{}) ([]byte, error) {
...
// ParseSlackCode 推送工单到slack解析插入代码
func ParseSlackCode(str string) string {

	preOpen, _ := regexp.Compile("<pre.*?>")
	str = preOpen.ReplaceAllString(str, "```")
	preClose := regexp.MustCompile("</pre>")
	str = preClose.ReplaceAllString(str, "```")
	// 将所有hmtl标签替换成换行符
...
// ParseSlackImg 推送工单到slack解析图片src
func ParseSlackImg(str string) []map[string]interface{} {
	...
	doc.Find("img").Each(func(i int, s *goquery.Selection) {
		...
		attachment["text"] = ""
		attachment["image_url"], _ = s.Attr("src")
