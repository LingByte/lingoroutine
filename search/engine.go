...
func (e *bleveEngine) Get(ctx context.Context, id string) (Hit, error) {
	if err := e.guard(); err != nil {
		return Hit{}, err
	}
	if id == "" {
		return Hit{}, errors.New("id is empty")
	}
	...
	err := e.withDeadline(ctx, e.cfg.QueryTimeout, func(ctx context.Context) error {
		...
		h := res.Hits[0]
		out = Hit{ID: h.ID, Score: h.Score, Fields: h.Fields, Fragments: h.Fragments}
		return nil
	})
	return out, err
}

func (e *bleveEngine) List(ctx context.Context, from int, size int) (SearchResult, error) {
	if err := e.guard(); err != nil {
		return SearchResult{}, err
	}
	if from < 0 {
	...
	out := SearchResult{Total: res.Total, Took: res.Took, Hits: make([]Hit, 0, len(res.Hits)), Facets: map[string]FacetResult{}}
	for _, h := range res.Hits {
		out.Hits = append(out.Hits, Hit{ID: h.ID, Score: h.Score, Fields: h.Fields, Fragments: h.Fragments})
	}
	return out, nil
}
